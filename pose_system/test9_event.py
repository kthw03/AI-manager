...
def main():
    ...
    try:
        while True:
            ...
            # posture analyzer 업데이트
            if landmarks is not None and primary_bbox is not None:
                analyzer.update(current_label, landmarks, primary_bbox)

            # 이상상황 dict 생성 (기본 false)
            anomalies = {
                "fall_detected": False,
                "prone_warning": False,
                "motionless": False,
                "irregular_movement": False,
                "tilt_sustained": False,
            }
            for ev in analyzer.get_events():
                if ev["type"] in anomalies:
                    anomalies[ev["type"]] = True

            # posture + anomalies Firebase 업로드
